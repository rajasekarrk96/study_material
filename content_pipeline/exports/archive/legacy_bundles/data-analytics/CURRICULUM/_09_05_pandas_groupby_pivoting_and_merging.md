...
metadata:
  ...
  lesson_slug: "pandas-groupby-pivoting-and-merging"
  lesson_title: "Lesson 2.3 GroupBy Aggregations, Pivoting, & Merging DataFrames"
  sort_order: 203
...
prerequisites:
  required_lesson_ids:
    - "DS-MOD02-LES02"
  required_skills:
    - "Pandas Indexing, Boolean Filtering, & Data Types"

skills_acquired:
  - "Mastering Split-Apply-Combine Aggregations (`groupby()`, `agg()`, `transform()`)"
  - "Constructing Reshaped Pivot Tables (`df.pivot_table()`, `pd.crosstab()`)"
  - "Executing Relational Joins & Merges (`pd.merge()`, inner, left, outer)"
  - "Concatenating Multi-Source DataFrames (`pd.concat()`)"
...
seo_and_social:
  meta_title: "Pandas Aggregations: groupby, agg, pivot_table & pd.merge Relational Joins"
  meta_description: "Master Pandas Aggregations & Joins: split-apply-combine with groupby(), custom agg() functions, pivot_table() reshaping, and pd.merge() relational joins."
  keywords: ["Pandas groupby", "Pandas agg", "pivot_table", "pd.merge", "Relational Joins Pandas", "pd.concat", "Split Apply Combine"]
...
# Lesson 2.3 GroupBy Aggregations, Pivoting, & Merging DataFrames
