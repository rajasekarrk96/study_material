...
metadata:
  ...
  lesson_slug: "numpy-core-architecture-and-ndarray"
  lesson_title: "Lesson 1.1 NumPy Core Architecture & ndarray Memory Layout"
  sort_order: 101
...
prerequisites:
  required_lesson_ids:
    - "DSM-MOD01-LES11"
  required_skills:
    - "Python 3.12 Fundamentals & Linear Algebra Vectors/Matrices"

skills_acquired:
  - "Understanding C-Contiguous Contiguous Memory Allocations in `ndarray`"
  - "Constructing N-Dimensional Arrays (`np.array()`, `np.zeros()`, `np.ones()`, `np.arange()`)"
  - "Inspecting Array Metadata Attributes (`shape`, `ndim`, `dtype`, `strides`, `itemsize`)"
  - "Manipulating Data Types & Memory Casting (`astype()`)"
...
seo_and_social:
  meta_title: "NumPy ndarray Architecture: C-Contiguous Memory, Strides & Dtypes"
  meta_description: "Master NumPy Core Architecture: ndarray C-contiguous memory layout, strides, dtypes, array creation routines, and memory efficiency vs Python lists."
  keywords: ["NumPy ndarray", "NumPy Architecture", "C-Contiguous Memory", "NumPy Strides", "dtype", "Vectorized Computing", "Python Data Science"]
