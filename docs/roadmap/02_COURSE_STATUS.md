# 02 — Master Course Inventory & Audit Status

> Complete audit breakdown of all **74 Master Courses** in Learning OS.  
> **Last Updated**: `2026-07-29 20:42:36`

---

| Status Icon | Meaning | Course Count |
|-------------|---------|-------------:|
| 🟢 Completed | Fully published in DB with complete markdown sections | 46 |
| 🟡 In Progress | Structure ready in DB (modules/lessons created) | 21 |
| 🔴 Pending | Placeholder stub in DB (needs structure seed) | 7 |

---

## 📚 Master Course List

| # | Course Title | Slug | Category / Domain | Difficulty | Lessons | Status | Completion % |
|---|--------------|------|-------------------|------------|--------:|--------|-------------:|
| 1 | **AWS** | `aws` | Amazon Web Services core servi... | Intermediate | 0/25 | 🟡 In Progress / Structure Ready | 0.0% |
| 2 | **Advanced Components** | `advanced-components` | Work with advanced IoT compone... | intermediate | 0/0 | 🔴 Pending / Stub | 0.0% |
| 3 | **Advanced Python** | `advanced-python` | Deep-dive into Python: decorat... | Intermediate | 0/30 | 🟡 In Progress / Structure Ready | 0.0% |
| 4 | **Ai Agents** | `ai-agents` | Comprehensive Ai Agents master... | beginner | 58/58 | 🟢 Completed & Published | 100.0% |
| 5 | **Arduino** | `arduino` | Get started with Arduino — GPI... | beginner | 50/50 | 🟢 Completed & Published | 100.0% |
| 6 | **Authentication & JWT** | `auth-jwt` | Implement secure authenticatio... | Intermediate | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 7 | **Bash Scripting** | `bash` | Automate system tasks with Bas... | Beginner | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 8 | **Basic MATLAB** | `basic-matlab` | Introduction to MATLAB for eng... | beginner | 0/0 | 🔴 Pending / Stub | 0.0% |
| 9 | **Basic ML for IoT** | `basic-ml-iot` | Deploy machine learning at the... | advanced | 0/0 | 🔴 Pending / Stub | 0.0% |
| 10 | **Bootstrap** | `bootstrap` | Comprehensive Bootstrap master... | beginner | 18/18 | 🟢 Completed & Published | 100.0% |
| 11 | **C Programming** | `c-programming` | Comprehensive C mastery course... | beginner | 18/18 | 🟢 Completed & Published | 100.0% |
| 12 | **C++ Object-Oriented Programming** | `c-object-oriented-programming` | Learn C++ Object-Oriented Prog... | beginner | 2/2 | 🟢 Completed & Published | 100.0% |
| 13 | **Computer Vision** | `computer-vision` | Comprehensive Computer Vision ... | beginner | 72/72 | 🟢 Completed & Published | 100.0% |
| 14 | **Computer Vision for IoT** | `computer-vision-iot` | Add vision intelligence to IoT... | advanced | 0/0 | 🔴 Pending / Stub | 0.0% |
| 15 | **Core Java** | `core-java` | Comprehensive Java programming... | intermediate | 160/160 | 🟢 Completed & Published | 100.0% |
| 16 | **Core Python** | `core-python` | Comprehensive Python mastery c... | beginner | 44/44 | 🟢 Completed & Published | 100.0% |
| 17 | **Cpp** | `cpp` | Comprehensive Cpp mastery cour... | beginner | 14/14 | 🟢 Completed & Published | 100.0% |
| 18 | **Css3** | `css3` | Comprehensive Css3 mastery cou... | beginner | 45/45 | 🟢 Completed & Published | 100.0% |
| 19 | **Data Structures & Algorithms** | `python-dsa` | Core DSA in Python — arrays, l... | Intermediate | 0/25 | 🟡 In Progress / Structure Ready | 0.0% |
| 20 | **Deep Learning** | `deep-learning` | Comprehensive Deep Learning ma... | beginner | 94/94 | 🟢 Completed & Published | 100.0% |
| 21 | **Docker** | `docker` | Containerize applications with... | Intermediate | 0/25 | 🟡 In Progress / Structure Ready | 0.0% |
| 22 | **Ds Math** | `ds-math` | Comprehensive Ds Math mastery ... | beginner | 11/11 | 🟢 Completed & Published | 100.0% |
| 23 | **ESP32** | `esp32` | Build WiFi and BLE IoT devices... | intermediate | 55/55 | 🟢 Completed & Published | 100.0% |
| 24 | **Electrical Fundamentals** | `electrical-fundamentals` | Essential electrical theory fo... | Beginner | 15/15 | 🟢 Completed & Published | 100.0% |
| 25 | **Electronics Basics** | `electronics-basics` | Core electronics for embedded ... | Beginner | 20/20 | 🟢 Completed & Published | 100.0% |
| 26 | **Embedded C** | `embedded-c` | Program microcontrollers using... | intermediate | 60/60 | 🟢 Completed & Published | 100.0% |
| 27 | **Fastapi** | `fastapi` | Comprehensive Fastapi mastery ... | beginner | 33/33 | 🟢 Completed & Published | 100.0% |
| 28 | **Firebase** | `firebase` | Build real-time IoT dashboards... | Beginner | 15/15 | 🟢 Completed & Published | 100.0% |
| 29 | **Flask** | `flask` | Comprehensive Flask mastery co... | beginner | 32/32 | 🟢 Completed & Published | 100.0% |
| 30 | **Generative Ai Llms** | `generative-ai-llms` | Comprehensive Generative Ai Ll... | beginner | 71/71 | 🟢 Completed & Published | 100.0% |
| 31 | **Git** | `git` | Comprehensive Git mastery cour... | beginner | 20/20 | 🟢 Completed & Published | 100.0% |
| 32 | **Git Fundamentals** | `git-fundamentals` | Learn Git Fundamentals from sc... | beginner | 238/238 | 🟢 Completed & Published | 100.0% |
| 33 | **GitHub Actions** | `github-actions` | CI/CD with GitHub Actions — wo... | Intermediate | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 34 | **Hibernate & JPA** | `hibernate` | Master Java persistence with H... | intermediate | 0/0 | 🔴 Pending / Stub | 0.0% |
| 35 | **Html5** | `html5` | Comprehensive Html5 mastery co... | beginner | 24/24 | 🟢 Completed & Published | 100.0% |
| 36 | **IoT Cloud** | `iot-cloud` | Connect IoT devices to the clo... | intermediate | 0/0 | 🔴 Pending / Stub | 0.0% |
| 37 | **Iot Hardware** | `iot-hardware` | Comprehensive Iot Hardware mas... | beginner | 68/68 | 🟢 Completed & Published | 100.0% |
| 38 | **Iot Projects** | `iot-projects` | Comprehensive Iot Projects mas... | beginner | 12/12 | 🟢 Completed & Published | 100.0% |
| 39 | **Java** | `java` | Comprehensive Java mastery cou... | beginner | 26/26 | 🟢 Completed & Published | 100.0% |
| 40 | **Java Selenium** | `java-selenium` | End-to-end browser automation ... | intermediate | 31/31 | 🟢 Completed & Published | 100.0% |
| 41 | **Javascript** | `javascript` | Comprehensive Javascript maste... | beginner | 52/52 | 🟢 Completed & Published | 100.0% |
| 42 | **Jenkins** | `jenkins` | Enterprise CI/CD with Jenkins ... | Intermediate | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 43 | **Jquery** | `jquery` | Comprehensive Jquery mastery c... | beginner | 12/12 | 🟢 Completed & Published | 100.0% |
| 44 | **Kubernetes** | `kubernetes` | Container orchestration with K... | Advanced | 0/20 | 🟡 In Progress / Structure Ready | 0.0% |
| 45 | **Linux Administration** | `linux` | Master the Linux command line ... | Beginner | 0/25 | 🟡 In Progress / Structure Ready | 0.0% |
| 46 | **MQTT Protocol** | `mqtt` | Implement lightweight IoT mess... | intermediate | 30/30 | 🟢 Completed & Published | 100.0% |
| 47 | **Machine Learning** | `machine-learning` | Comprehensive Machine Learning... | beginner | 107/107 | 🟢 Completed & Published | 100.0% |
| 48 | **Manual Testing** | `manual-testing` | Software QA fundamentals — SDL... | Beginner | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 49 | **Maven** | `maven` | Java build automation with Mav... | Beginner | 0/10 | 🟡 In Progress / Structure Ready | 0.0% |
| 50 | **Mlops Ai Deployment** | `mlops-ai-deployment` | Comprehensive Mlops Ai Deploym... | beginner | 59/59 | 🟢 Completed & Published | 100.0% |
| 51 | **Mongodb** | `mongodb` | Comprehensive Mongodb mastery ... | beginner | 13/13 | 🟢 Completed & Published | 100.0% |
| 52 | **MySQL** | `mysql` | Relational database design, qu... | beginner | 53/53 | 🟢 Completed & Published | 100.0% |
| 53 | **Nlp** | `nlp` | Comprehensive Nlp mastery cour... | beginner | 72/72 | 🟢 Completed & Published | 100.0% |
| 54 | **Pcb** | `pcb` | Comprehensive Pcb mastery cour... | beginner | 28/28 | 🟢 Completed & Published | 100.0% |
| 55 | **Playwright** | `playwright` | Modern end-to-end testing with... | Intermediate | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 56 | **Postman / API Testing** | `postman` | API testing with Postman — col... | Beginner | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 57 | **Power Bi** | `power-bi` | Comprehensive Power Bi mastery... | beginner | 35/35 | 🟢 Completed & Published | 100.0% |
| 58 | **Prompt Engineering** | `prompt-engineering` | Comprehensive Prompt Engineeri... | beginner | 39/39 | 🟢 Completed & Published | 100.0% |
| 59 | **Python Data Science** | `python-data-science` | Comprehensive Python Data Scie... | beginner | 5/5 | 🟢 Completed & Published | 100.0% |
| 60 | **REST API Development** | `rest-api` | Design and build professional ... | Intermediate | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 61 | **Rag Engineering** | `rag-engineering` | Comprehensive Rag Engineering ... | beginner | 58/58 | 🟢 Completed & Published | 100.0% |
| 62 | **Raspberry Pi** | `raspberry-pi` | Use Raspberry Pi as a Linux-ba... | intermediate | 15/15 | 🟢 Completed & Published | 100.0% |
| 63 | **React.js** | `react` | Build modern single-page appli... | Intermediate | 0/30 | 🟡 In Progress / Structure Ready | 0.0% |
| 64 | **STM32** | `stm32` | Professional embedded developm... | Advanced | 25/25 | 🟢 Completed & Published | 100.0% |
| 65 | **Selenium** | `selenium` | Comprehensive Selenium mastery... | beginner | 25/25 | 🟢 Completed & Published | 100.0% |
| 66 | **Sensors & Actuators** | `sensors-actuators` | Interface real-world sensors (... | beginner | 35/35 | 🟢 Completed & Published | 100.0% |
| 67 | **Servlet & JSP** | `servlet-jsp` | Java web fundamentals — HTTP S... | Intermediate | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 68 | **Simulation (Proteus / Wokwi)** | `simulation` | Design and simulate electronic... | beginner | 0/0 | 🔴 Pending / Stub | 0.0% |
| 69 | **Spring Boot** | `spring-boot` | Build enterprise-grade Java we... | intermediate | 0/20 | 🟡 In Progress / Structure Ready | 0.0% |
| 70 | **Spring Framework** | `spring` | Core Spring — IoC container, d... | Intermediate | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 71 | **Spring MVC** | `spring-mvc` | Build web applications with Sp... | Intermediate | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 72 | **Spring Security** | `spring-security` | Secure Spring Boot application... | Advanced | 0/15 | 🟡 In Progress / Structure Ready | 0.0% |
| 73 | **Sql Server** | `sql-server` | Comprehensive Sql Server maste... | beginner | 37/37 | 🟢 Completed & Published | 100.0% |
| 74 | **TinyML** | `tinyml` | Deploy machine learning on mic... | Advanced | 15/15 | 🟢 Completed & Published | 100.0% |
