...
metadata:
  ...
  lesson_slug: "jwt-authentication-and-current-user"
  lesson_title: "Lesson 5.2 JWT Authentication & Current User Dependency"
  sort_order: 502
...
prerequisites:
  required_lesson_ids:
    - "FAP-MOD05-LES01"
  required_skills:
    - "FastAPI OAuth2PasswordBearer & Dependency Injection"

skills_acquired:
  - "Encoding & Decoding JWT Tokens with PyJWT (`jwt.encode`, `jwt.decode`)"
  - "Configuring Token Expiration (`exp` claims)"
  - "Constructing the `get_current_user` Dependency Pipeline"
  - "Role-Based Access Control (RBAC) with Sub-Dependencies"
...
seo_and_social:
  meta_title: "FastAPI JWT Authentication: PyJWT, get_current_user Dependency & RBAC"
  meta_description: "Master JWT Authentication in FastAPI: encoding/decoding tokens with PyJWT, building get_current_user dependency pipeline, and Role-Based Access Control (RBAC)."
  keywords: ["FastAPI JWT", "PyJWT", "get_current_user", "JWT Token", "FastAPI RBAC", "Role Based Access Control"]
